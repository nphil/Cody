...
// PNG. The capture happens where the dev server actually runs, so it works
// even when the viewer's browser cannot resolve the app as localhost.
export async function POST(req: Request) {
  try {
    ...
    if (error instanceof ScreenshotError) {
      const status = error.code === "invalid_url" ? 400 : error.code === "chromium_missing" ? 503 : 502;
