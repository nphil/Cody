...
test("Fable — the Claude tier above Opus — brands as Claude in every spelling", () => {
  // Direct, bare ids (the two real shapes: "claude-fable-5" and "claude-fable-5-1").
  assert.equal(modelBrand("anthropic", "claude-fable-5").icon, "claude");
  assert.equal(modelBrand("anthropic", "claude-fable-5-1").icon, "claude");
  // Effort-suffixed variants some gateways expose as separate selectable ids.
  assert.equal(modelBrand("anthropic", "claude-fable-5-1-xhigh").icon, "claude");
  assert.equal(modelBrand("anthropic", "claude-fable-5-1-max").icon, "claude");
  // Dotted aggregator spelling, behind a gateway provider — the vendor prefix
  // states itself before the family regex is ever consulted.
  assert.equal(modelBrand("openrouter", "anthropic/claude-fable-5.1").icon, "claude");
  // The same dotted id with no vendor prefix at all still matches on family.
  assert.equal(modelBrand("openrouter", "claude-fable-5.1").icon, "claude");
});
