...
function assertPrivateFileMode(filePath) {
  // POSIX mode bits are not enforced by Node's `mode` option on Windows;
  // Windows uses the account ACL instead. Keep the 0600 contract on systems
  // where these bits are meaningful without declaring a Windows false failure.
...
test("user store round-trips records with 0600 and unique usernames", async () => {
  ...
  assertPrivateFileMode(storePath);
