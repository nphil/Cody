...
const OMP_IMPORT_ALLOWLIST = {
  ...
  "app/api/auth/api-key/[provider]/route.ts": "omp utility RPC: api-key status",
  "app/api/auth/login/[provider]/route.ts": "omp RPC login flow (extension_ui_request)",
  "app/api/auth/providers/route.ts": "omp utility RPC: login providers",
  ...
  "app/api/models/route.ts": "rpc-dialect utility RPC: available models (engine-aware via utilityRpcLaunchFor)",
  ...
  "app/api/omp-settings/schema/route.ts": "omp settings schema reader",
  "app/api/omp-update/route.ts": "explicitly omp-scoped update check",
  "app/api/omp-version/route.ts": "explicitly omp-scoped version probe",
  ...
  "lib/skills-service.ts": "skill discovery mirrors omp's provider order",
  ...
  "lib/session-namer.ts": "names sessions with a one-shot omp CLI run on the omp `tiny` model role",
 ...
 * `state`, `engine-bin`, `engine-sessions`, `install`, `updates`, and the
 * `@/lib/harness` index are the public engine-neutral surface; these are not. */
const PRIVATE_HARNESS_MODULES = new Set(["omp", "pi", "claude", "codex", "hermes", "claude-stream", "codex-stream", "acp-session"]);
/** EngineCommandError is engine-neutral but lives in turn-session.ts; the two
 * agent routes import it. Anything else must go through the public surface. */
const TURN_SESSION_ALLOWLIST = new Set(["app/api/agent/new/route.ts", "app/api/agent/[id]/route.ts"]);
    ...
    for (const file of walk(join(ROOT, dir))) {
      ...
      for (const spec of importSpecs(file)) {
        ...
        if (!rel.startsWith("lib/harness/")) {
          ...
          if (sub === "turn-session" && !TURN_SESSION_ALLOWLIST.has(rel)) harnessViolations.push(`${rel} -> ${target}`);
        }
      }
    }
  }
