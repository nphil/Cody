...
const OMP_IMPORT_ALLOWLIST = {
  ...
  "lib/distill/runner.ts": "spawns the installed omp binary for each chain attempt (the rpc dialect's print-mode CLI, the same one-shot mechanism the session namer uses), in an isolated agent dir shared with it via lib/omp/isolated-agent-dir.ts",
