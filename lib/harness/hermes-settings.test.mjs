...
test("credential-shaped string leaves are flagged secret; everything else is not", async () => {
  ...
  const settings = flattenHermesDefaults({
    ...
    dashboard: { basic_auth: { password: "", secret: "", enabled: false } },
    telegram: { token: "" },
    openai: { apiKey: "" },
    // A number named like a budget is not a credential, and neither is a
    // key that merely CONTAINS the word.
    agent: { max_tokens: 5, token_budget: 3, tokenizer: "tiktoken" },
  });
  const byKey = Object.fromEntries(settings.map((s) => [s.key, s]));
  for (const key of ["auxiliary.vision.api_key", "dashboard.basic_auth.password", "dashboard.basic_auth.secret", "telegram.token", "openai.apiKey"]) {
    assert.equal(byKey[key].secret, true, `${key} is a secret leaf`);
    assert.equal(byKey[key].type, "string");
  }
  for (const key of ["auxiliary.vision.provider", "dashboard.basic_auth.enabled", "agent.max_tokens", "agent.token_budget", "agent.tokenizer"]) {
