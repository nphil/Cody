 ...
 * Use a Node-backed wrapper on Windows: extensionless shell scripts are not
 * executable there, and the production adapter supports the same .cmd shape. */
function stubHermes({ logPath = null, stderr = null, exitCode = 0 }) {
  const dir = mkdtempSync(join(tmpdir(), "hermes-cli-"));
  const modulePath = join(dir, "hermes-stub.mjs");
  writeFileSync(modulePath, [
    'import { appendFileSync } from "node:fs";',
    "const args = process.argv.slice(2);",
    logPath ? `appendFileSync(${JSON.stringify(logPath)}, args.join(" ") + "\\n");` : "",
    stderr ? `process.stderr.write(${JSON.stringify(`${stderr}\\n`)});` : "",
    `process.exit(${exitCode});`,
  ].filter(Boolean).join("\\n"));
  if (process.platform === "win32") {
    const commandPath = join(dir, "hermes.cmd");
    writeFileSync(commandPath, `@echo off\\r\\n"${process.execPath}" "%~dp0hermes-stub.mjs" %*\\r\\n`);
    return commandPath;
  }
  const commandPath = join(dir, "hermes");
...
test("the schema is re-read after a reinstall rebuilds the venv", { skip: process.platform === "win32" ? "POSIX Hermes/Python fixture is not executable on Windows" : false }, async () => {
...
test("reset unsets the key, and a key with no override is already reset", { skip: process.platform === "win32" ? "POSIX Hermes CLI fixture is not executable on Windows" : false }, () => {
  const log = join(mkdtempSync(join(tmpdir(), "hermes-log-")), "args");
  const ok = stubHermes({ logPath: log });
  ...
  const absent = stubHermes({ stderr: "Config key not set: agent.max_turns", exitCode: 1 });
  resetHermesSetting(absent, "agent.max_turns");

  // Every other refusal still surfaces, carrying the CLI's own reason.
  const managed = stubHermes({ stderr: "Cannot set: managed by your administrator", exitCode: 1 });
...
test("credential-shaped string leaves are flagged secret; everything else is not", async () => {
  const { isHermesSecretKey } = await jiti.import("./hermes-settings.ts");
  const settings = flattenHermesDefaults({
    auxiliary: { vision: { api_key: "", provider: "auto" } },
    dashboard: { basic_auth: { password: "", secret: "", enabled: false, password_hash: "" } },
    browser: { camofox: { session_key: "" } },
    telegram: { token: "" },
    openai: { apiKey: "" },
    portal: { session_cookie: "" },
    // A number named like a budget is not a credential, and neither is a
    // key that merely CONTAINS the word, nor a hotkey ending in `key`.
    agent: { max_tokens: 5, token_budget: 3, tokenizer: "tiktoken" },
    keybindings: { record_key: "ctrl+b" },
  });
  const byKey = Object.fromEntries(settings.map((s) => [s.key, s]));
  for (const key of [
    ...
    // An offline-crackable scrypt hash: not plaintext, still a credential.
    "dashboard.basic_auth.password_hash",
    // An externally managed browser identity key, not an "...api_key".
    "browser.camofox.session_key",
    "telegram.token",
    "openai.apiKey",
    "portal.session_cookie",
  ]) {
    assert.equal(byKey[key].secret, true, `${key} is a secret leaf`);
    assert.equal(byKey[key].type, "string");
  }
  for (const key of ["auxiliary.vision.provider", "dashboard.basic_auth.enabled", "agent.max_tokens", "agent.token_budget", "agent.tokenizer", "keybindings.record_key"]) {
    assert.equal(byKey[key].secret, undefined, `${key} is not a secret leaf`);
  }
  // The rule is name AND type: a number can never be a credential here.
  assert.equal(isHermesSecretKey("x.api_key", "number"), false);
  assert.equal(isHermesSecretKey("x.api_key", "string"), true);
  assert.equal(isHermesSecretKey("dashboard.basic_auth.password_hash", "string"), true);
  assert.equal(isHermesSecretKey("browser.camofox.session_key", "string"), true);
  assert.equal(isHermesSecretKey("keybindings.record_key", "string"), false, "a bare `key` suffix stays editable");
  // The read side still returns the value: redaction is the ROUTE's job, so
  // the adapter stays a faithful mirror of the file.
  const home = mkdtempSync(join(tmpdir(), "hermes-home-"));
  writeFileSync(join(home, "config.yaml"), "auxiliary:\n  vision:\n    api_key: sk-live-123\n");
  assert.equal(readHermesSettingsValues(home, settings)["auxiliary.vision.api_key"], "sk-live-123");
  rmSync(home, { recursive: true, force: true });
});
