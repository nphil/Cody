...
test("a write preserves everything Cody does not list, and resets prune", () => {
  ...
  if (process.platform !== "win32") assert.equal(statSync(file).mode & 0o777, 0o640, "the file's existing mode survives the write");
...
| \`auth.passwordHash\` | string | - | Stored password hash |
| \`browser.sessionKey\` | string | - | Browser session identity |
| \`portal.sessionCookie\` | string | - | Portal session cookie |
| \`retry.maxTokens\` | number | \`5\` | Not a credential, a budget |
| \`tokenizer\` | string | \`"tiktoken"\` | Contains the word, is not one |
| \`keybindings.recordKey\` | string | \`"ctrl+b"\` | A hotkey, not a credential |
`;
  const keys = byKey(parsePiSettingsDocs(docs));
  for (const key of ["providerApiKey", "auth.token", "webhookSecret", "proxyPassword", "auth.passwordHash", "browser.sessionKey", "portal.sessionCookie"]) {
  ...
  assert.equal(keys["keybindings.recordKey"].secret, undefined, "a bare `key` suffix stays editable");
  assert.equal(isPiSecretKey("providerApiKey", "number"), false, "name AND type: a number is never a credential");
  assert.equal(isPiSecretKey("auth.passwordHash", "string"), true);
  assert.equal(isPiSecretKey("browser.sessionKey", "string"), true);
  assert.equal(isPiSecretKey("keybindings.recordKey", "string"), false);
