...
function fakeHermes(dir, body) {
  const modulePath = path.join(dir, `fake-hermes-${fakeCount++}.mjs`);
  fs.writeFileSync(modulePath, `process.stdout.write(${JSON.stringify(body)});\nprocess.exit(0);\n`);
  if (process.platform === "win32") {
    const commandPath = path.join(dir, `fake-hermes-${fakeCount++}.cmd`);
    fs.writeFileSync(commandPath, `@echo off\r\n"${process.execPath}" "%~dp0${path.basename(modulePath)}" %*\r\n`);
    return commandPath;
  }
  const commandPath = path.join(dir, `fake-hermes-${fakeCount++}`);
