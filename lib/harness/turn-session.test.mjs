...
const MINTED_THREAD_ID = "0199f0b1-thread";
...
/**
 * A transcript from an engine that MINTS ITS OWN session id and reports the
 * turn's tokens once, at the end — the other half of the turn-engine contract
 * from Claude Code's (Cody pre-assigns the id, usage arrives per message).
 *
 * Deliberately a local fixture rather than a real engine's translator: the
 * branches under test belong to TurnEngineSession, and pinning them to one
 * engine's stream module meant they went with it when that engine migrated to
 * another transport.
 */
const MINTED_TURN = [
  { type: "session_opened", id: MINTED_THREAD_ID },
  { type: "text", text: "Done." },
  { type: "finished", usage: { input: 3, output: 4 } },
];

function createMintedState(seed = { engineSessionId: null, model: null }) {
  return {
    engineSessionId: seed.engineSessionId,
    model: seed.model,
    provider: "openai",
    modelFallback: "minted-model",
    text: "",
    thinking: "",
    streaming: false,
    toolNames: new Map(),
...
function translateMintedLine(line, state) {
  if (!line || typeof line !== "object") return [];
  switch (line.type) {
    case "session_opened":
      state.engineSessionId = line.id;
      return [];
    case "text":
      return [{
        type: "message_end",
    ...
    case "finished": {
      const usage = { input: line.usage.input, output: line.usage.output, cacheRead: 0, cacheWrite: 0 };
      state.usage = { ...usage };
      return [{ type: "usage_event", usage }];
    }
    default:
      return [];
  }
...
test("a prompt runs one child turn and emits the full pi event sequence", async () => {
  ...
  assert.deepEqual(seen.types(), [
    ...
    "tool_execution_end",
    "usage_event", // the result frame's first-party cost
    "agent_end",
  ]);
...
test("both engines report usage over the live event stream, where they used to report nothing", async () => {
  // Claude Code: the recorded turn carries no per-message usage, so the result
  // frame's own price for the turn is the whole account — and it reaches the
  // browser instead of being parsed into a field nobody read.
  const claude = makeSpec({ id: "claude-usage" });
  const claudeSession = new TurnEngineSession(claude.spec, { sessionId: "usage-claude", cwd: process.cwd() });
  claudeSession.start();
  const claudeSeen = collector(claudeSession);
  await claudeSession.send({ type: "prompt", message: "list the files" });
  await claudeSeen.waitFor("agent_end");
  const claudeUsage = claudeSeen.events.filter((event) => event.type === "usage_event");
  assert.deepEqual(claudeUsage.map((event) => event.usage), [
    { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, cost: 0.01 },
  ]);
  await claudeSession.destroyAndWait();

  // Two turns of the same claude session accumulate rather than restate: the
  // browser adds every frame, so a second turn must not resend the first's.
  const twoTurn = makeSpec({ id: "claude-usage-2" });
  const repeated = new TurnEngineSession(twoTurn.spec, { sessionId: "usage-claude-2", cwd: process.cwd() });
  repeated.start();
  const repeatedSeen = collector(repeated);
  await repeated.send({ type: "prompt", message: "first" });
  await repeatedSeen.waitFor("agent_end", 1);
  await repeated.send({ type: "prompt", message: "second" });
  await repeatedSeen.waitFor("agent_end", 2);
  const repeatedCosts = repeatedSeen.events
    .filter((event) => event.type === "usage_event")
    .map((event) => event.usage.cost);
  assert.deepEqual(repeatedCosts, [0.01, 0.01]);
  await repeated.destroyAndWait();

  // The other shape: one terminal frame carrying the whole turn's tokens,
  // forwarded as a single additive usage event.
  const minted = makeSpec({
    id: "minted-usage",
    provider: "openai",
    defaultModel: "minted-model",
    preassignsIdentity: false,
    lines: MINTED_TURN,
    createState: createMintedState,
    translate: translateMintedLine,
  });
  const mintedSession = new TurnEngineSession(minted.spec, { sessionId: "", cwd: process.cwd() });
  mintedSession.start();
  const mintedSeen = collector(mintedSession);
  await mintedSession.send({ type: "prompt", message: "say done" });
  await mintedSeen.waitFor("agent_end");
  assert.deepEqual(
    mintedSeen.events.filter((event) => event.type === "usage_event").map((event) => event.usage),
    [{ input: 3, output: 4, cacheRead: 0, cacheWrite: 0 }],
  );
  await mintedSession.destroyAndWait();
});
...
test("an engine that mints its own id re-keys the session and its index row", async () => {
  ...
  assert.match(localId, /^fake-minted-/);
  ...
  assert.deepEqual(renames, [[localId, MINTED_THREAD_ID]]);
  assert.equal(session.sessionId, MINTED_THREAD_ID);
  assert.equal(index.getEngineSession(localId), null);
  const row = index.getEngineSession(MINTED_THREAD_ID);
  assert.equal(row.engine, "fake-minted");
  assert.equal(row.engineSessionId, MINTED_THREAD_ID);
