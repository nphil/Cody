...
test("both engines report usage over the live event stream, where they used to report nothing", async () => {
  ...
  // Codex: `turn.completed` is its only usage report, and it arrives as one
  // additive frame with the cached/cache-write split already resolved.
  const codex = makeSpec({
    id: "codex-usage",
    provider: "openai",
    defaultModel: "codex",
  ...
  const codexSession = new TurnEngineSession(codex.spec, { sessionId: "", cwd: process.cwd() });
  codexSession.start();
  const codexSeen = collector(codexSession);
  await codexSession.send({ type: "prompt", message: "say done" });
  await codexSeen.waitFor("agent_end");
  assert.deepEqual(
    codexSeen.events.filter((event) => event.type === "usage_event").map((event) => event.usage),
    [{ input: 3, output: 4, cacheRead: 0, cacheWrite: 0 }],
  );
  await codexSession.destroyAndWait();
});
