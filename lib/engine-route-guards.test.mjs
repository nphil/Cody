...
test("the schema route always carries secretsSet, and a secret leaf's value never leaves the server", { skip: process.platform === "win32" ? "POSIX Hermes fixture is not executable on Windows" : false }, async () => {
  ...
  const hermesName = process.platform === "win32" ? "hermes.cmd" : "hermes";
  const pythonName = process.platform === "win32" ? "python.cmd" : "python";
  const hermesCommand = process.platform === "win32"
    ? `@echo off\r\nexit /b 0\r\n`
    : "#!/bin/sh\nexit 0\n";
  fs.writeFileSync(path.join(bin, hermesName), hermesCommand, { mode: 0o755 });
  const defaults = JSON.stringify({ auxiliary: { vision: { api_key: "", provider: "auto" } }, agent: { max_turns: 90 } });
  const pythonModule = path.join(bin, "python-stub.mjs");
  fs.writeFileSync(pythonModule, `process.stdout.write(${JSON.stringify(defaults)});\n`);
  const pythonCommand = process.platform === "win32"
    ? `@echo off\r\n"${process.execPath}" "%~dp0python-stub.mjs" %*\r\n`
    : `#!/bin/sh\nexec ${JSON.stringify(process.execPath)} ${JSON.stringify(pythonModule)} "$@"\n`;
  fs.writeFileSync(path.join(bin, pythonName), pythonCommand, { mode: 0o755 });
  ...
  process.env.CODY_HERMES_BIN = path.join(bin, hermesName);
