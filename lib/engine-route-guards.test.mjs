...
test("the two settings routes are gated by the two different flags they answer to", async () => {
  ...
  // Code and Codex — none of which declare nativeSettings — got omp's ~550-key
  // schema and omp's config.yml values back, stamped with their own shortName.
  // The PUT was the real damage: it fell through to omp's writer and saved
  // into omp's config.yml while another engine was active, reporting success.
