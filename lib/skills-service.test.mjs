...
test("scan roots cover the compat directories the app installs into", () => {
  ...
  // Setting CLAUDE_CONFIG_DIR is itself omp's opt-in for that user-level source
  // (isUserSourceEnabled), so ~/.claude/skills is expected below; ~/.codex/skills
  // is opt-in only and has its own test.
...
/** Scan roots for a fresh instance whose omp config.yml holds `config`. The cwd
 * is a project directory well away from home, so a user-level root and its
 * project-level namesake cannot be confused for one another. */
function rootsForConfig(config) {
  const dir = mkdtempSync(join(tmpdir(), "cody-skill-gate-"));
  const agentDir = join(dir, ".omp", "agent");
  const previousAgentDir = process.env.PI_CODING_AGENT_DIR;
  const previousClaudeDir = process.env.CLAUDE_CONFIG_DIR;
  mkdirSync(agentDir, { recursive: true });
  if (config) writeFileSync(join(agentDir, "config.yml"), config, "utf8");
  process.env.PI_CODING_AGENT_DIR = agentDir;
  delete process.env.CLAUDE_CONFIG_DIR;
  try {
    return { roots: getSkillScanRootDirs(join(dir, "project")), project: join(dir, "project") };
  } finally {
    if (previousAgentDir === undefined) delete process.env.PI_CODING_AGENT_DIR;
    else process.env.PI_CODING_AGENT_DIR = previousAgentDir;
    if (previousClaudeDir !== undefined) process.env.CLAUDE_CONFIG_DIR = previousClaudeDir;
    rmSync(dir, { recursive: true, force: true });
  }
}

// omp 18.1.5 made another tool's ~/ skills directory opt-in and defaults both
// flags to false. Listing a root omp does not read offers skills the session
// cannot invoke, so Cody's walk has to follow the same rule.
test("foreign user-level skill roots are omitted until omp is opted in", () => {
  const { roots, project } = rootsForConfig("");
  assert.equal(roots.includes(join(homedir(), ".claude", "skills")), false, "~/.claude/skills is opt-in");
  assert.equal(roots.includes(join(homedir(), ".codex", "skills")), false, "~/.codex/skills is opt-in");
  // Project-level compat dirs are unaffected by those flags and still load.
  assert.ok(roots.includes(join(project, ".claude", "skills")), "project .claude/skills still scanned");
  assert.ok(roots.includes(join(project, ".codex", "skills")), "project .codex/skills still scanned");
});

test("the per-source flag opts a foreign user root back in", () => {
  const { roots } = rootsForConfig("skills:\n  enableCodexUser: true\n");
  assert.ok(roots.includes(join(homedir(), ".codex", "skills")), "~/.codex/skills returns when enabled");
  assert.equal(roots.includes(join(homedir(), ".claude", "skills")), false, "the other source stays opt-in");
});

test("enabledProviders opts a foreign user root back in", () => {
  // omp's own second door: `isUserSourceEnabled` admits a provider listed in
  // enabledProviders even with the per-source flag off.
  const { roots } = rootsForConfig("enabledProviders:\n  - claude\n");
  assert.ok(roots.includes(join(homedir(), ".claude", "skills")), "~/.claude/skills returns when the provider is enabled");
  assert.equal(roots.includes(join(homedir(), ".codex", "skills")), false, "the other source stays opt-in");
});
