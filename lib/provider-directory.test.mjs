...
test("credential bridge availability reason follows the login into its provider method", () => {
  const reason = "credential_list_failed: Installed OMP API is unavailable.";
  const annotatedLogins = logins.map((login) => login.id === "anthropic" ? { ...login, accountDetailsReason: reason } : login);
  const anthropic = ompRows({ logins: annotatedLogins }).find((row) => row.id === "anthropic");
  assert.equal(anthropic.methods.find((method) => method.loginId === "anthropic")?.accountDetailsReason, reason);
});
