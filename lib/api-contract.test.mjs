...
test("settings groups runtime preferences and resource managers behind tabs", async () => {
  ...
  // Update surfaces are consolidated into the System hub.
  ...
  assert.match(enginePanel, /Enable Advisor for new sessions/);
  // omp's registry editor is still reachable (under Providers until the
  // catalog lands).
  assert.match(providersPanel, /<ModelsConfig embedded/);
