...
test("settings groups runtime preferences and resource managers behind tabs", async () => {
  ...
  const registry = await readFile(new URL("../components/settings/registry.ts", import.meta.url), "utf8");
  const systemPanel = await readFile(new URL("../components/settings/panels/SystemPanel.tsx", import.meta.url), "utf8");
  const enginePanel = await readFile(new URL("../components/settings/panels/EnginePanel.tsx", import.meta.url), "utf8");
  const recommendedCards = await readFile(new URL("../components/settings/engine/recommended-cards.ts", import.meta.url), "utf8");
  const providersPanel = await readFile(new URL("../components/settings/panels/ProvidersPanel.tsx", import.meta.url), "utf8");
  const updates = await readFile(new URL("../components/settings/SystemUpdates.tsx", import.meta.url), "utf8");
  const roster = await readFile(new URL("../components/settings/EngineRoster.tsx", import.meta.url), "utf8");
  // The dialog is a wrapper over the shell; the hubs are the registry's rows.
  assert.match(settings, /SettingsShell/);
  for (const id of ["system", "engine", "providers", "extensions", "models"]) {
    assert.match(registry, new RegExp(`id: "${id}"`), `registry lists the ${id} hub`);
  }
  // Update surfaces are consolidated into the System hub: the Cody card
  // over the engine roster, which owns the copyable-command card and the
  // session-restart control (the onboarding picker renders the same roster).
  assert.match(systemPanel, /SystemUpdates/);
  assert.match(updates, /EngineRoster/);
  assert.match(roster, /updates\.runCommandHint/);
  assert.match(roster, /updates\.omp\.restart/);
  // Advisor, model defaults and approvals live under Behavior.
  // The Enable Advisor card is bound to omp's own schema row; the card table
  // is what pins it now, not a hand-written label in the panel.
  assert.match(enginePanel, /RecommendedSettings/);
  assert.match(recommendedCards, /"advisor\.enabled"/);
  // Providers is the directory over /api/providers (sign-ins, keys and
  // custom endpoints in one list), not the old registry tree.
  assert.match(providersPanel, /ProviderDirectory/);
  // The catalog read moved into the Models hub's data hook; the registry
  // editor no longer fetches it.
  const catalogHook = await readFile(new URL("../hooks/useModelCatalog.ts", import.meta.url), "utf8");
  assert.match(catalogHook, /"\/api\/models"/);
});
