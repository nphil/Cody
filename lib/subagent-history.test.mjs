...
test("does not follow artifact symlinks outside the sibling directory", (t) => {
  const dir = mkdtempSync(join(tmpdir(), "cody-subagent-history-"));
  const sessionFile = join(dir, "sess.jsonl");
  const artifactsDir = siblingDirForSession(sessionFile);
  ...
  try {
    ...
    try {
      ...
      if (process.platform === "win32" && error?.code === "EPERM") return t.skip("creating symlinks requires Windows developer mode or elevated privileges");
      throw error;
    }
