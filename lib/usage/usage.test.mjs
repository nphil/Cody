...
test("selectBindingWindow ranks state above utilization", () => {
  const exhausted = window({ id: "exhausted", utilization: 100, state: "exhausted" });
  const warning = window({ id: "warning", utilization: 95, state: "warning" });
  const ok = window({ id: "ok", utilization: 60, state: "ok" });
  ...
  assert.equal(withoutExhausted.window.id, "warning");
});
...
test("selectBindingWindow picks the binding window of a real snapshot", () => {
  ...
  // Two windows sit at 100% exhausted, so the soonest reset decides.
...
test("a model is scoped to its own provider, not the worst provider on the box", () => {
  ...
  assert.equal(picked.account.provider, "anthropic");
  // Most binding first: 14% > 9% > 7%, all of them below any threshold.
