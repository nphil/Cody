...
const { RpcProcess, RpcCommandError, RpcCommandTimeoutError } = jiti("./rpc-process.ts");
...
test("RpcProcess rejects timed-out commands and removes their pending entry", async () => {
  const transport = makeTransport();
  const proc = startProcess(transport);
  await proc.waitReady();

  const pending = proc.sendCommand({ type: "never_returns" }, 20);
  const error = await pending.then(() => null, (thrown) => thrown);

  assert.ok(error instanceof RpcCommandTimeoutError);
  assert.equal(error.command, "never_returns");
