...
const accountRoute = await readFile(new URL("../app/api/auth/account/[provider]/route.ts", import.meta.url), "utf8");
...
test("single-account removal advertises permanent semantics and naming is admin-gated", () => {
  assert.match(logoutRoute, /permanently removes just that one stored credential/);
  assert.match(logoutRoute, /permanent: true/);
  assert.match(accountRoute, /export async function PATCH/);
  assert.match(accountRoute, /requireAdmin\(req\)/);
  assert.match(accountRoute, /requireCapability\("providerLogin", "Provider account names"\)/);
  assert.match(accountRoute, /renameAccount\(provider, accountId/);
  assert.match(accountRoute, /Cody(?:'s own| only) metadata store/);
});
