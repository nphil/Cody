...
test("curation can unhide selected models without changing other personal hides", () => {
  assert.deepEqual(
    lib.unhideModels(["z/9", "a/1", "z/9", "b/2"], ["b/2", "a/1"]),
    ["z/9"],
  );
  assert.deepEqual(lib.unhideModels(["a/1"], ["missing/1"]), ["a/1"]);
});
