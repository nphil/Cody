...
test("builds an archive that python extracts back to the fixture bytes", { skip: hasPython() ? false : `${pythonCommand} is not installed` }, async (t) => {
  ...
  execFileSync(pythonCommand, ["-m", "zipfile", "-t", zipPath], { stdio: "pipe" });

  // Full extraction round-trip.
  const extracted = path.join(root, "extracted");
  execFileSync(pythonCommand, ["-m", "zipfile", "-e", zipPath, extracted], { stdio: "pipe" });
